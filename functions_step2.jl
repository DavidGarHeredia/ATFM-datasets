...
function create_sector_and_route_information!(input::Input, DF_airports::DataFrame)
  ...
  # Graph of routes
  DF_graph, dictAirportNode = create_graph_of_routes(input, DF_airports, pointsMiddle, Δrow, Δcol);
...
function create_graph_of_routes(input::Input, 
  ...
  dictSectorPoints = fill_df_graph_with_connections!(input, DF_graph, pointsMiddle, Δrow, Δcol);
  dictAirportNode = fill_df_graph_with_airports_connections!(dictSectorPoints, DF_graph, DF_airports);
